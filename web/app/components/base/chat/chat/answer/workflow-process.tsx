  ...
  return (
    <div
      className={cn(
        ...
        collapse
          ? "py-[7px] border-components-panel-border"
          : "pt-[7px] px-1 pb-1 border-components-panel-border-subtle"
      )}
      style={{
        background
      }}
    >
      ...
      >
        {running && <RiLoader2Line className="shrink-0 mr-1 w-3.5 h-3.5 text-text-tertiary" />}
        {succeeded && <CheckCircle className="shrink-0 mr-1 w-3.5 h-3.5 text-text-success" />}
        {failed && (
        ...
        <RiArrowRightSLine
          className={`'ml-1 w-4 h-4 text-text-tertiary' ${collapse ? "" : "rotate-90"}`}
        />
      </div>
      {!collapse && (
