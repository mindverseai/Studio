...
const TextGeneration: FC<IMainProps> = ({
  ...
  const [_batchCompletionRes, doSetBatchCompletionRes] = useState<
  ...
  useEffect(() => {
    if (siteInfo?.title) {
      ...
      else document.title = `${siteInfo.title} - Powered by Mindverse`;
    }
        ...
        <div
          className={`${resWrapClassNames} ${
            !isPC && isShowResSidebar
              ? styles.resultWrapperMobile
              : styles.resultWrapperDefault
          }`}
        >
          {renderResWrap}
        </div>
