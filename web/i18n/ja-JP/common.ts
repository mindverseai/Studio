const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'Mindverse アカウント',
    langGeniusAccountTip: 'あなたの Mindverse アカウントと関連するユーザーデータ。',
    editName: '名前を編集',
  ...
  members: {
    ...
    invitationSentTip: '招待が送信されました。Mindverse にログインしてチームデータにアクセスできます。',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      desc: 'Mindverse が提供する OpenAI ホスティングサービスを使用すると、GPT-3.5 などのモデルを使用できます。トライアルクォータが消費される前に、他のモデルプロバイダーを設定する必要があります。',
      callTimes: '通話回数',
  ...
  modelProvider: {
    notConfigured: '���ステムモデルがまだ完全に設定されておらず、一部の機能が利用できない場合があります。',
    ...
    loadBalancingInfo: 'デフォルトでは、負荷分散はラウンドロビン方式を採用しています。レート制限が��生した場合、1分間のクールダウン期間が適用されます。',
  ...
  about: {
    changeLog: '変更ログ',
    updateNow: '今すぐ更新',
    nowAvailable: 'Mindverse {{version}} が利用可能になりました。',
    latestAvailable: 'Mindverse {{version}} が最新バージョンです。',
  },
  appMenus: {
    overview: '監視',
    promptEng: 'オーケストレート',
  ...
  imageUploader: {
    ...
    pasteImageLinkInvalid: '無効���画像リンク',
